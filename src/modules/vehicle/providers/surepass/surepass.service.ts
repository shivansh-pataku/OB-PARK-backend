...
export class SurepassService {
  ...
  constructor(private readonly configService: ConfigService) {
    this.baseUrl = this.configService.getOrThrow<string>('surepass.baseUrl');

    this.token = this.configService.getOrThrow<string>('surepass.token');
  }
  ...
  async lookupRc(vehicleNumber: string): Promise<SurepassRcResponse> {
    return this.request<SurepassRcResponse>('/api/v1/rc/rc-full', {
  ...
  async lookupRcV2(body: SurepassRcV2Request): Promise<SurepassRcV2Response> {
    return this.request<SurepassRcV2Response>('/api/v1/rc/rc-v2', body);
  }
}
