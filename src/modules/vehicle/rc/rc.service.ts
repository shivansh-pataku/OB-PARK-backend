...
export class RcService {
  ...
  async lookupVehicle(dto: RcRequestDto): Promise<RcResponseDto> {
    const response = await this.surepassService.lookupRc(dto.vehicleNumber);
  ...
  async lookupChallan(dto: RcChallanRequestDto): Promise<RcChallanResponseDto> {
    ...
    return {
      ...
      challans: (response.data.challan_details.challans || []).map(
        (challan) => ({
  ...
  async lookupChallanAdvanced(
    ...
    return {
      ...
      challans: (response.data.challan_details || []).map((challan) => ({
        ...
        offenseDetails: challan.offense_details,
        place: challan.challan_place,
        offenseDetailsList: (challan.offense_details_list || []).map(
          (offense) => ({
  ...
  async lookupRcV2(dto: RcV2RequestDto): Promise<RcV2ResponseDto> {
